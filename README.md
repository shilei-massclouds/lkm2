...
根目录 `make run` 与 `make -C tools run` 默认显式使用
...
1。`make run USER_RUNTIME_SIGNALS=tools/signals/default.signals` 可显式选择相同行为；
`make run USER_RUNTIME_SIGNALS=` 会省略信号文件参数，也回退到推导器的内存默认程序。
