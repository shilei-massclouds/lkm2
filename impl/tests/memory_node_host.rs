...
fn memblock(memory_ranges: &[(u64, u64)]) -> (MemBlock, Vec<u8>) {
    ...
    let block = MemBlock::setup_bootmem(
        memory,
        &dtb,
        0x8040_0000,
        (0x8010_0000, 0x1000),
...
fn node_envelope_and_mem_map_follow_memblock() {
    ...
    assert_eq!(node.layout().physical_envelope(), (0x8000_1003, 0x9000_3000));
...
fn reservations_are_removed_from_each_zone_managed_projection() {
    ...
    assert!(managed.iter().all(|(start, end)| {
        *start >= 0x8000_0000 && *end <= 0x8800_0000 && *start < *end
    }));
    assert!(managed.iter().all(|(start, end)| {
        *end <= 0x8010_0000 || *start >= 0x8010_1000
    }));
