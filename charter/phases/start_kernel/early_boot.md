...
再依次驱动 `EarlyConsole.Enable` 和 `Cpu0Scheduler.Enable`，最后以
...
current 且 OnCpu，Banner、DtbBlob、EarlyConsole 和 `Cpu0Scheduler` 必须为 Online，且
`BootCommandLine` 必须存在 `earlycon` 键，但 EarlyBoot 不断言其具体外部输入值；
