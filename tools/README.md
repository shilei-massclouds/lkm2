...
```text
model/main.spec → entry AST → recursive module graph → Model IR v15 → canonical JSON
entry external signals → tools/build/derive/main.sequence.json
Model IR v15 + external-root sequence v3 → derive → result JSON v12 / human stdout
```

有状态对象可以省略 `initial_state`；modelc 会将其规范化为
`State::Base`。因此这类对象必须声明 `state State::Base`。显式指定的初始状态保持
不变，无状态对象仍没有初始状态。Model IR v15 的严格 JSON schema 中：
...
Model IR v15 `entry.spec` 中的主根。当前模型由 `systems`、`objects`、
...
定义的语法；未知关键字、未知声明和错误语法都会报错。Model IR v15 lowering
...
类型体还可以用 `parent: ParentType;` 声明父类型契约；契约随类型继承传递，
实例化时必须绑定到兼容的具体对象，类型体中的 `parent` 表达式会在展开对象时静态替换。
...
自动调度。结果 JSON schema v12 顶层记录有序路径集合；每条路径保留嵌套推导单元、
...
Model IR v15 还提供内建 `String`、`Relation<K,V>`、`Map<K,V>` 和一个有序
...
`name := expression;` binding 右侧，binding 可供后续条件、signal 实参、updates
右侧、ensures、establishes 与 emits 实参使用。Relation 允许同 key 多值；Map
重复同一 tuple 幂等，同 key 不同值在任何 drive 前以 `map_key_conflict` 原子失败。
result v12 为每个 unit 保存类型化 binding/effect，并为每条 path 保存按
owner/key/value 规范排序的最终 tuple 快照；continuation frame 同样保存类型化
witness，恢复时不会重新 lookup。
...
schema 为 v15。InterruptFlow 受每 CPU `Unknown/Masked/Unmasked` gate 控制，masked 输入
